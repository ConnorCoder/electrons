...
function ePush(element) {
  ...
  h.appendChild(document.createTextNode(`(${elements.indexOf(element) + 1}) ${element} - ${eConfig(elements.indexOf(element) + 1)} - ${eAbbrev(elements.indexOf(element) + 1)}`));
...
function eConfig(atomic) {
  ...
  let total = [2, 4, 10, 12, 18, 20, 30, 36, 38, 48, 54, 56, 70, 80, 86, 88, 102, 112, 118];
  let assigned = {s: 2,p: 6,d: 10,f: 14}
  let orbits = [];
  let highest = 0;
  let diff = 0;
  let index = 0;
